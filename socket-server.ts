...
*/
    ...
    socket.on('wannaStart', async (roomName: RoomName) => {
      ...
      if (room != undefined) {
        ...
        if (polishedTeams.slice(1).length < 3) {
          socket.emit('gameStatus', { status: false, reason: "Hacen falta al menos 3 equipos para empezar la partida" });
        }
        else if (tooLargeTeam) {
          socket.emit('gameStatus', { status: false, reason: "No puede haber un equipo con más de 2 miembros" });
        }
        else {
  ...
  async function startGame(room: Room, game: Game, gameDataEmitter: EventEmitter) {
    ...
    let roundTimer: TimerStatus = setRoundCountdown(room,  gameDataEmitter, { i: true, t: 5 });
    ...
    for (let teamNumber of game.data.drawingOrder) {
      ...
      //Tiempo pre-ronda
      roundTimer = setRoundCountdown(room, gameDataEmitter, { i: true, t: 3 });
      await roundTimer.status;

      if (gameEnded) break;
      //Tiempo de la ronda
      roundTimer = setRoundCountdown(room, gameDataEmitter, { i: false, t: 60 || defaultGameConfig.roundDuration }, game, teamNumber);
      await roundTimer.status;
    }
  ...
  function setRoundCountdown(room: Room, gameDataEmitter: EventEmitter, startingTime: TimeData, game?: Game, teamNumber?: TeamNumber): TimerStatus {
    ...
    let correctWordCallback = (senderId: UserID) => {
      ...
      if (game != undefined && teamNumber != undefined && wordNumber < 10) {
        ...
        if (senderTeam != -1)
          game.dynamicData.score[senderTeam - 1] += startingTime.t;
        game.dynamicData.score[teamNumber - 1] += startingTime.t;
        game.dynamicData.currentWord = game.data.words[teamNumber - 1][wordNumber];
        game.dynamicData.wordStatus = game.dynamicData.currentWord.replace(/[^!¡.]/g, '_');
